...
ID=$(curl -s https://api.github.com/repos/alttch/modbusgw/releases | jq -r .\[0\].id)
UPLOAD_ID=$(curl -X POST -H "Authorization: token ${GITHUB_TOKEN}" \
    -H "Content-Type: $MIME_TYPE" \
    --data-binary @${FILE} \
    "https://uploads.github.com/repos/alttch/modbusgw/releases/${ID}/assets?name=$1" | jq -r .id)
[ -z "$UPLOAD_ID" ] && exit 1
